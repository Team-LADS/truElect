...
describe("TruElectToken and TruElectVoting Contract Deployment...", function(){
    ...
    it("Should mint tokens to address",async function(){
        ...
        minted = await tokenContract.connect(owner).mint(secondAddress.address,1);
    ...
    it("Should not mint tokens to voters if caller is not the election committee head or member of the election committee",async function(){
        const [owner,secondAddress,thirdAddress,fourthAddress,fifthAddress,sixthAddress,seventhAddress,eightAddress,ninthAddress,tenthAddress] = await ethers.getSigners();
        // await expect(tokenContract.connect(fourthAddress).mintToVoter(1,"electionCommittee",[eightAddress.address,secondAddress.address,tenthAddress.address])).to.be.revertedWith("Access granted to only the election committee head or a member of the election committee");
        await expect(tokenContract.connect(fourthAddress).mintToVoter(1,[eightAddress.address,secondAddress.address,tenthAddress.address])).to.be.revertedWith("Access granted to only the election committee head or a member of the election committee");
...
describe("Voting For a Candidate Category ...",function(){
    ...
    it("Should not be able to vote if candidate is not registered for an office",async function(){
       ...
       await expect(votingContract.connect(owner).vote("chancellor",1)).to.be.revertedWith("Candidate is not Registered for this Office!");
       console.log('\t',"Passed ....")
    });
    ...
    it("Should not be able to vote token is less than threshold of 1*10**18",async function(){
        ...
        console.log('\t',"Passed ....")
        mintToAddress = await tokenContract.connect(owner).mint(secondAddress.address,ethers.utils.parseEther("1"));
...
describe("Change election committee head role... and concensus voting",function(){
    ...
    it("Should revert if address is a non voter",async function(){
        const [owner,secondAddress,thirdAddress,fourthAddress,fifthAddress,sixthAddress,seventhAddress,eightAddress,ninthAddress] = await ethers.getSigners();
        console.log('\t',"Attempting to Change election committee head role when caller is not a member of election committee....");
        await expect(votingContract.connect(eightAddress).changeElectionCommHead(ninthAddress.address)).to.be.revertedWith("Only the election committee have access")
    ...
    it("Should revert if votes to remove committee head is less than 80% from the member of the election committee",async function(){
        ...
        await expect(votingContract.connect(eightAddress).changeElectionCommHead(ninthAddress.address)).to.be.revertedWith("Requires Greater than 80% consent of members of the election committee to approve!") 
        console.log('\t',"Passed ....");
    });
